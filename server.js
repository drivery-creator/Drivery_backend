...
// --- NÚCLEO LOGÍSTICO DRIVERY ---
app.post('/api/command', async (req, res) => {
    let { command, userCoords, session, credentials } = req.body;
    let newSessionGenerated = null;

    try {
        // 1. IA + Tasa en Paralelo
        const [completion, tasaBCV] = await Promise.all([
            groq.chat.completions.create({
                messages: [{ role: "system", content: "Extrae destino en Caracas. JSON: {\"destino\": \"Lugar\"}" }, { role: "user", content: command }],
        ...
        // 2. Geocoding con límites tácticos
        ...
        // 3. PROCESAMIENTO DE TODA LA FLOTA
        // Extraemos todos los tipos de vehículos disponibles (Eco, Plus, XL, etc.)
        ...
        res.json({
            ...
            origin: userCoords,
            reply: `Análisis de despliegue para ${destinoNombre} completado.`,
            display: { fleet: fleetData, tasa: tasaBCV },
            newSession: newSessionGenerated
        });

    } catch (e) {
        res.status(401).json({ reply: "Re-sincronización de flota requerida." });
    }
});
...
app.listen(PORT, () => console.log(`DRIVERY CORE TURBO-FLOTA ONLINE`));
