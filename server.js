...
// Endpoint para procesar el comando e interceptar flota
app.post('/api/command', async (req, res) => {
    ...
    try {
        ...
        res.json({
            destCoords, 
            reply: `Ruta a ${destinoNombre} sincronizada. Seleccione unidad para proceder.`,
            display: { fleet: fleetData }
        });
    } catch (e) { res.status(500).json({ reply: "Error de red táctica. Reintente." }); }
});
...
app.post('/api/book', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, reply: "Confirmado. Unidad asignada y en camino." });
    } catch (e) { res.status(500).json({ success: false, reply: "Fallo en el despliegue de unidad." }); }
});
